...
// CHECK-NEXT:     }
// CHECK-NEXT:     %[[VAL_4:.*]] = ktdp.inter_tile_reduce(%[[VAL_2]]) consumer_tiles_per_group = #[[$ATTR_0]], identity(%[[VAL_1]] : tensor<1x64xf16>) : <(tensor<1x64xf16>), groups = #[[$ATTR_1]]> -> tensor<64xf16> {
...
// CHECK-NEXT:       ktdp.yield_partial %[[VAL_0]] : tensor<1x64xf16>
// CHECK-NEXT:     }
// CHECK-NEXT:     %[[VAL_4:.*]] = ktdp.inter_tile_reduce(%[[VAL_2]]) consumer_tiles_per_group = #[[$ATTR_3]], identity(%[[VAL_1]] : tensor<1x64xf16>) : <(tensor<1x64xf16>), groups = #[[$ATTR_1]]> -> tensor<64xf16> {
// CHECK-NEXT:     ^bb0(%[[VAL_5:.*]]: tensor<1x64xf16>, %[[VAL_6:.*]]: tensor<1x64xf16>):
// CHECK-NEXT:       %[[VAL_7:.*]] = linalg.add ins(%[[VAL_5]], %[[VAL_6]] : tensor<1x64xf16>, tensor<1x64xf16>) outs(%[[VAL_5]] : tensor<1x64xf16>) -> tensor<1x64xf16>
// CHECK-NEXT:       ktdp.yield_reduced %[[VAL_7]] : tensor<1x64xf16>
// CHECK-NEXT:     }
...
// CHECK-NEXT:     }
// CHECK-NEXT:     %[[VAL_4:.*]] = ktdp.inter_tile_reduce(%[[VAL_2]]) consumer_tiles_per_group = #[[$ATTR_2]], identity(%[[VAL_1]] : tensor<128x1x1x64xf16>) : <(tensor<128x1x1x64xf16>), groups = #[[$ATTR_4]]> -> tensor<128x1x64xf16> {
...
// CHECK-NEXT:     }
// CHECK-NEXT:     return %[[VAL_4]] : tensor<128x1x64xf16>
// CHECK-NEXT:   }
...
// CHECK-NEXT:     }
// CHECK-NEXT:     %[[VAL_6:.*]]:2 = ktdp.inter_tile_reduce(%[[VAL_4]]) consumer_tiles_per_group = #[[$ATTR_2]], producer_dependency_per_consumer = #[[$ATTR_5]], identity(%[[VAL_2]], %[[VAL_3]] : tensor<1x64xf32>, tensor<1x64xi32>) : <(tensor<1x64xf32>, tensor<1x64xi32>), groups = #[[$ATTR_4]]> -> tensor<64xf32>, tensor<64xi32> {
...
// CHECK-NEXT:     }
// CHECK-NEXT:     return %[[VAL_14:.*]]#0, %[[VAL_14]]#1 : tensor<64xf32>, tensor<64xi32>
// CHECK-NEXT:   }
...
func.func @reduce_single_role(%partial: tensor<1x64xf16>,
  ...
  %r = ktdp.inter_tile_reduce(%f)
      ...
      : <(tensor<1x64xf16>), groups = #all_groups> -> tensor<64xf16>
  {
...
func.func @reduce_to_one(%partial: tensor<1x64xf16>,
  ...
  %r = ktdp.inter_tile_reduce(%f)
      ...
      identity(%add_id : tensor<1x64xf16>)
      : <(tensor<1x64xf16>), groups = #r2o_grp> -> tensor<64xf16>
  { ^bb0(%lhs: tensor<1x64xf16>, %rhs: tensor<1x64xf16>):
      %s = linalg.add ins(%lhs, %rhs : tensor<1x64xf16>, tensor<1x64xf16>)
                      outs(%lhs : tensor<1x64xf16>) -> tensor<1x64xf16>
      ktdp.yield_reduced %s : tensor<1x64xf16> }
...
func.func @reduce_multi_group(%partial: tensor<128x1x1x64xf16>,
  ...
  %r = ktdp.inter_tile_reduce(%f)
      ...
      : <(tensor<128x1x1x64xf16>), groups = #mg_groups> -> tensor<128x1x64xf16>
  {
